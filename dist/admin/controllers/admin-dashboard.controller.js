...
let AdminDashboardController = class AdminDashboardController {
    ...
    async getUserDetails(id, session) {
        ...
        try {
            ...
            const totalSpentResult = await purchaseRepository
                ...
                .where('purchase.user_id = :userId', { userId })
                .getRawOne();
            const casinoActions = await this.usersService.getUserCasinoActions(user.visitor_id);
            return {
                ...user,
    ...
    async getUsersList(options) {
        try {
            const isCasinoActionSort = options.sortBy === 'casino_registrations' ||
            ...
            if (isCasinoActionSort) {
                ...
                const casinoActionsCounts = await this.usersService.getCasinoActionsCounts(visitorIds);
                ...
                const paginatedUsers = usersWithCasinoActions.slice(skip, skip + options.limit);
                return {
                    data: paginatedUsers,
                    pagination: this.buildPagination(options.page, options.limit, usersWithCasinoActions.length),
                };
            }
            else {
