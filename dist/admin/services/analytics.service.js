...
let AnalyticsService = class AnalyticsService {
    ...
    constructor(playerRepository, playHistoryRepository, purchaseRepository, balanceChangeRepository, rpBalanceRepository, casinoActionRepository) {
    ...
    async getAllDeposits(dateRange) {
    ...
    async getAllRegistrations(dateRange) {
        let query = this.casinoActionRepository
            .createQueryBuilder('action')
    ...
    async getUniqueDeposits(dateRange) {
        let query = this.casinoActionRepository
            .createQueryBuilder('action')
            .select('COUNT(DISTINCT CONCAT(action.visitor_id, action.casino_name))', 'count')
            .where('action.deposit = :deposit', { deposit: true });
        if (dateRange?.startDate) {
            query = query.andWhere('action.date_of_action >= :startDate', {
                startDate: dateRange.startDate,
    ...
    async getUniqueRegistrations(dateRange) {
        let query = this.casinoActionRepository
            .createQueryBuilder('action')
            .select('COUNT(DISTINCT CONCAT(action.visitor_id, action.casino_name))', 'count')
            .where('action.registration = :registration', { registration: true });
...
exports.AnalyticsService = AnalyticsService = __decorate([
    ...
    __param(5, (0, typeorm_1.InjectRepository)(casino_action_entity_1.CasinoAction)),
